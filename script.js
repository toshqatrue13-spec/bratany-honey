...
var revState = { reviews:[], page:0, selectedStar:0 };
...
function revInit(){
  var stored=[];
  try{ stored=JSON.parse(localStorage.getItem('bratany_reviews')||'[]'); }catch(e){}
  revState.reviews = SEED_REVIEWS.concat(stored);
  revRender();
  revUpdateStats();
}
...
function submitReview(){
  ...
  fetch('https://formspree.io/f/xeevzpen',{
    method:'POST',
    headers:{'Content-Type':'application/json','Accept':'application/json'},
    body:JSON.stringify({_subject:'Новий відгук від '+name,type:'review',name:name,city:city,rating:stars+' ('+rating+'/5)',product:product,review:text})
  })
  .then(function(res){
    btn.disabled=false;
    btn.textContent='Надіслати відгук 🍯';
    if(res.ok){
      var newRev={id:'u_'+Date.now(),name:name,city:city,rating:rating,product:product,text:text,date:new Date().toISOString().slice(0,10),isNew:true};
      var stored=[];
      try{stored=JSON.parse(localStorage.getItem('bratany_reviews')||'[]');}catch(e){}
      stored.push(newRev);
      localStorage.setItem('bratany_reviews',JSON.stringify(stored));
      revState.reviews=SEED_REVIEWS.concat(stored);
      revState.page=0;
      revRender();
      revUpdateStats();
      ...
      errEl.textContent='Помилка відправки. Спробуйте ще раз.';
  ...
  .catch(function(){
    btn.disabled=false;
    btn.textContent='Надіслати відгук 🍯';
    errEl.textContent='Немає зв\'язку. Перевірте інтернет і спробуйте знову.';
    errEl.style.display='block';
  });
}
