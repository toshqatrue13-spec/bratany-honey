...
var PRODUCTS = {
  ...
  'acacia_mix': {
    ...
    desc:`<p>Акація з аморфою — гармонійне поєднання ніжності акації та яскравого аромату аморфи..</p>
    <ul>
      <li>Склад: акація + аморфа\'я</li>
  ...
  'may': {
    emoji:'🍯',image:'trav_ripak.png', title:'Травневий мед (з ріпаком)',
  ...
  'alpine-frame': {
   emoji:'🍯', image:'comb_frame_full_square_dark.jpg', title:"Альпійська рамка (лісовий мед)",
    ...
    prices:{'500 г':500}, // ⚠️ ціна орієнтовна — постав свою, коли визначишся
    taste:{Солодкість:80,Аромат:95,Густота:70,Насиченість:90},
    desc:`<p>Альпійська рамка — це мед прямо в стільниках, зібраний бджолами в лісових передгір'ях. Найчистіша форма меду: без відкачки, без фільтрації, без жодного втручання — так, як він визрів у вулику.</p>
    <ul>
      <li>Подається прямо в дерев'яній рамці — стільники їстівні</li>
      <li>Лісове різнотрав'я — насичений, багатогранний смак</li>
      <li>Максимум ферментів і пилку — нічого не втрачено при обробці</li>
      <li>Ідеально для подарунка або дегустаційного столу</li>
    </ul>`,
    health:[
      {icon:'🌲',title:"Лісове різнотрав'я",desc:"Бджоли збирають нектар з дикорослих лісових квітів — унікальний, насичений мінеральний склад."},
      {icon:'🍯',title:"Мед у природному вигляді",desc:"Без нагрівання і фільтрації — збережені всі ферменти, віск і прополіс зі стільників."},
      {icon:'🦷',title:"Корисно жувати віск",desc:"Стільниковий віск природно очищує ротову порожнину та масажує ясна."},
      {icon:'🛡️',title:"Прополіс у складі",desc:"Сліди прополісу зі стільників мають антибактеріальну дію."},
      {icon:'💪',title:"Енергія та витривалість",desc:"Природні цукри у поєднанні з пилком дають тривалий заряд енергії."},
      {icon:'🎁',title:"Подарунковий формат",desc:"Ефектна подача в дерев'яній рамці — вражає з першого погляду."},
    ]
  },
  'drone': {
...
var PRODUCT_KEYS = ['acacia_pure','acacia_mix','may','wildflower','linden','rapeseed','sunflower','pollen','comb','alpine-frame','drone','buckwheat','royal'];
...
if (p.image) {
    modalEmoji.innerHTML = `<img src="${p.image}" style="width:170px;height:170px;object-fit:cover;border-radius:16px;">`;
} else {
